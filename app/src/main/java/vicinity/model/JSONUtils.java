 ...
 *
 *
 * Ruba: We should put this code in the document and reference it I did not edit it
 *
 *
 */
...
public class JSONUtils {
    ...
    public static JSONObject getJsonObject( String jsonstr ) {
        ...
        try{
            ...
            Log.i(TAG, "getJsonObject : " + e.toString());
        }
        return jsonobj;
    }
    ...
    public static JSONArray getJsonArray(String jsonstr) {
        ...
        try {
            ...
            Log.i(TAG, "getJSONArray:" + e.toString());
        }
        return curjsons;
    }
    ...
    public static JSONArray truncateJSONArray(JSONArray origarray, int offset){
        ...
        if( sz > Globals.MSG_SIZE){
            ...
            try{
                ...
                Log.i(TAG, "truncateJSONArray :" + e.toString());
            }
            return newarray;
        }else{
    ...
    public static int indexOfJSONObject(JSONArray jsonarray, JSONObject jsonobj, String key) {
        ...
        if (key == null) {
            ...
            try {
                ...
                objstr = null;
                Log.i(TAG, "findJSONObject:  get key Exception: " + e.toString());
            }
        }
        ...
        if (objstr != null) {
            ...
            Log.i(TAG, "findJSONObject:  empty key string! no found. ");
            return -1;
        }
        ...
        for (int i=0; i<size; i++) {
            try {
                ...
                if (objstr.equals(entrystr)) {
                    Log.i(TAG, "findJSONObject: match :" + objstr);
                    return i;   // return immediately
                }
            } catch (JSONException e) {
                Log.i(TAG, "findJSONObject: getJSONObject Exception: " + e.toString());
                continue;
            }
        }
        return -1;
    ...
    public static JSONArray mergeJsonArrays(JSONArray existingjsons, JSONArray newjsons, boolean updatess) {
        ...
        for (int i=0; i<newjsons.length(); i++) {
            try {
                ...
                String sender = newobj.getString(Globals.MSG_SENDER);
                if(sender == null) {
                    continue;  // bad new json object, skip
                }
                int idx = indexOfJSONObject(existingjsons, newobj, Globals.MSG_SENDER);
                if(idx < 0) { // not found, insert newjson obj into json array
                    ...
                    if(newobj.has(Globals.MSG_SENDER)) {
                        String newss = newobj.getString(Globals.MSG_SENDER);
                        JSONObject oldobj = existingjsons.getJSONObject(idx);
                        oldobj.put(Globals.MSG_SENDER, newss);
                        Log.i(TAG, "mergeJsonArrays: update ss: " + newss + " : " + oldobj.toString());
                    }
                }
            } catch (JSONException e) {
                Log.i(TAG, "mergeJSONArrays: getJSONObject Exception: " + e.toString());
                continue;
            }
        }
        return existingjsons;
    ...
    public static String mergeJsonArrayStrings(String curstr, String newstr) {
        ...
        Log.i(TAG, "mergeJSONArrays:" + curstr + " =+= " + newstr);
        ...
        try {
            ...
            Log.i(TAG, "mergeJSONArrays:" + e.toString());
    ...
    public static boolean fuzzyMatchJsonArrays(String dbJsonStr, String curJsonStr, String key) {
        Log.i(TAG, "fuzzyMatchJSONArrays : dbsdbjsonstret : " + dbJsonStr + " : curjsonstr :" +curJsonStr);
    ...
    public static Set<String> getValueSetFromJsonArray(JSONArray jsonarray, String key) {
        ...
        for (int i=0; i<jsonarray.length(); i++) {
            try {
                ...
                Log.i(TAG, "getValueSetFromJSONArray: Exception: " + e.toString());
                continue;  // skip this entry if can not construct object.
            }
        }
        return valset;
        ...
        if (jsonString != null && jsonString.length() > 0) {
            for (String s : set) {
                ...
                if (jsonString.indexOf(wrapstr) >= 0) {
                    Log.i(TAG, "intersectSetJsonArray: Match: " + wrapstr);
                    matchcnt++;
                }
            }
        }
